...
class MainActivity : AppCompatActivity(), EventBus.Subscriber {
	...
	override fun onResume() {
		...
		EventBus.dispatch(Event(Type.MODE_CHANGE, Target.ACTIVITY, mapOf("mode" to
	...
	private fun initWebView() {
		...
		webView?.apply {
			...
			loadUrl("https://appassets.androidplatform.net/assets/index.html?mode=$mode") // Using standard domain
		}
	}
	...
	override fun handle(event: Event) {
		...
		when (event.type) {
			...
			Type.LIST_DIR -> {
				val path = event.data["path"] as? String ?: EXTERNAL_STORAGE_PATH
				val items = FileSystem.list(path)
				val responseData = mapOf(
					"path" to path,
					"items" to items.map { mapOf("name" to it.name, "path" to it.path, "isDirectory" to it.isDirectory) }
				)
				EventBus.dispatch(Event("dir_list", "app", responseData))
			}
		}
	}
	...
	private val permissionRequest = registerForActivityResult(ActivityResultContracts.RequestPermission()) { isGranted: Boolean ->
		EventBus.dispatch(Event(Type.MODE_CHANGE, Target.ACTIVITY, mapOf("mode" to if (isGranted) "normal" else "permission")))
	}
